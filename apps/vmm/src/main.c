...
static void handler_loop(void)
{
    ...
    for (;;) {
        ...
        // if (have_reply) {
        //     tag = api_reply_recv(INPUT_CAP, reply_tag, &badge, REPLY_CAP);
        // } else if (have_signal) {
        //     tag = api_nbsend_recv(signal, signal_msg, INPUT_CAP, &badge, REPLY_CAP);
        //     have_signal = false;
        // } else {
        //     tag = api_recv(INPUT_CAP, &badge, REPLY_CAP);
        // }
