...
#include <gpivmm/vgic/vgic_v2.h>
#include <gpivmm/vgic/vdist.h>
...
bool vgic_inject_irq(vm_context_t *vm, size_t vcpu_id, int irq)
{
    ...
    return vgic_dist_set_pending_irq(vm, &vgic, vcpu_id, irq);
...
bool handle_vgic_dist_fault(vm_context_t *vm, size_t vcpu_id,
                            uint64_t fault_addr, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    {
        success = vgic_dist_reg_read(vm, vcpu_id, &vgic, offset, fsr, regs);
        assert(success);
    }
    else
    {
        success = vgic_dist_reg_write(vm, vcpu_id, &vgic, offset, fsr, regs);
        assert(success);
    }
