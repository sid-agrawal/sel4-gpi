...
static void vgic_dist_enable_irq(vm_context_t *vm, vgic_t *vgic, size_t vcpu_id, int irq)
{
...
static bool vgic_dist_set_pending_irq(vm_context_t *vm, vgic_t *vgic, size_t vcpu_id, int irq)
{
...
#error "Unsupported GIC version"
    ...
    return vgic_vcpu_load_list_reg(vm, vgic, vcpu_id, idx, group, virq);
}
...
static bool vgic_dist_reg_read(vm_context_t *vm, size_t vcpu_id, vgic_t *vgic, uint64_t offset, uint64_t fsr, seL4_UserContext *regs)
{
    bool success = false;
    ...
    default:
        ...
        success = fault_advance_vcpu(vm, regs);
        assert(success);
        goto fault_return;
    }
    ...
    success = fault_advance(vm, regs, GIC_DIST_PADDR + offset, fsr, reg & mask);
    assert(success);
...
static bool vgic_dist_reg_write(vm_context_t *vm, size_t vcpu_id, vgic_t *vgic,
                                uint64_t offset, uint64_t fsr, seL4_UserContext *regs)
{
    ...
    case RANGE32(GIC_DIST_ISENABLER0, GIC_DIST_ISENABLERN):
        ...
        while (data)
        {
            ...
            vgic_dist_enable_irq(vm, vgic, vcpu_id, irq);
        }
        break;
    ...
    case RANGE32(GIC_DIST_ISPENDR0, GIC_DIST_ISPENDRN):
        ...
        while (data)
        {
            ...
            vgic_dist_set_pending_irq(vm, vgic, vcpu_id, irq);
        }
        break;
    ...
    case RANGE32(GIC_DIST_SGIR, GIC_DIST_SGIR):
        ...
        success = vgic_inject_irq(vm, vcpu_id, virq);
        assert(success);
        break;
    case RANGE32(0xF04, 0xF0C):
        /* Reserved */
