...
// @ivanv: revisit
static inline void virq_ack(vm_context_t *vm, struct virq_handle *irq)
{
    VMM_PRINTV("VGIC|INFO: Acking for vIRQ %d\n", irq->virq);
    assert(irq->ack_fn);
    irq->ack_fn(vm, irq->virq, irq->ack_data);
}
...
static inline bool vgic_vcpu_load_list_reg(vm_context_t *vm, vgic_t *vgic, size_t vcpu_id,
