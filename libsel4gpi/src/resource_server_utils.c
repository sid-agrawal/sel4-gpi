...
int resource_server_main(void *context_v)
{
    ...
    RESOURCE_SERVER_PRINTF("Server ready to receive requests\n");
    ...
    while (1)
    {
        ...
        RESOURCE_SERVER_PRINTF("Received message, passing to request handler\n");
                                          ...
                                          uint64_t resource_id, uint64_t client_id)
{
    ...
    int error = pd_client_give_resource(context->pd_conn, client_id,
