/**
 * @file cpu_parentapi.c    
 * @author Sid Agrawal(sid@sid-agrawal.c)
 * @brief Implements functions needed by a parent to interact with the cpu server from cpu_parentapi.h
 * @version 0.1
 * @date 2022-04-05
 * 
...
seL4_Error
cpu_server_parent_spawn_thread(simple_t *parent_simple, vka_t *parent_vka,
                                  ...
                                  seL4_CPtr *server_ep_cap)
{
    ...
    memset(get_cpu_server(), 0, sizeof(cpu_server_context_t));

    /* Get a CPtr to the parent's root cnode. */
    vka_cspace_make_path(parent_vka, 0, &parent_cspace_cspath);

    get_cpu_server()->server_vka = parent_vka;
    get_cpu_server()->server_vspace = parent_vspace;
    get_cpu_server()->server_cspace = parent_cspace_cspath.root;
    get_cpu_server()->server_simple = parent_simple;

    /* Allocate the Endpoint that the server will be listening on. */
    error = vka_alloc_endpoint(parent_vka, &get_cpu_server()->server_ep_obj);
    if (error != 0) {
        ZF_LOGE(CPUSERVP"spawn_thread: failed to alloc endpoint, err=%d.",
                error);
        return error;
    }

    *server_ep_cap = get_cpu_server()->server_ep_obj.cptr; 
    ...
    get_cpu_server()->parent_badge_value = CPU_SERVER_BADGE_PARENT_VALUE;

    error = vka_mint_object(parent_vka, &get_cpu_server()->server_ep_obj,
                            &get_cpu_server()->_badged_server_ep_cspath,
                            seL4_AllRights,
                            get_cpu_server()->parent_badge_value);
    if (error != 0) {
        ZF_LOGE(CPUSERVP"spawn_thread: Failed to mint badged Endpoint cap to "
                "server.\n"
    ...
    sel4utils_thread_config_t config = thread_config_default(parent_simple,
                                                             ...
                                                             seL4_NilData,
                                                             get_cpu_server()->server_ep_obj.cptr,
                                                             priority);
    error = sel4utils_configure_thread_config(parent_vka,
                                              parent_vspace,
                                              parent_vspace,
                                              config,
                                              &get_cpu_server()->server_thread);
    if (error != 0) {
        ZF_LOGE(CPUSERVP"spawn_thread: sel4utils_configure_thread failed "
                "with %d.", error);
        goto out;
    }

    NAME_THREAD(get_cpu_server()->server_thread.tcb.cptr, "cpu server");
    error = sel4utils_start_thread(&get_cpu_server()->server_thread,
                                   (sel4utils_thread_entry_fn)&cpu_server_main,
                                   NULL, NULL, 1);
    if (error != 0) {
        ZF_LOGE(CPUSERVP"spawn_thread: sel4utils_start_thread failed with "
                "%d.", error);
        goto out;
    }
    ...
    if (error != 0) {
        ZF_LOGE(CPUSERVP"spawn_thread: Server thread failed to bind to the "
                "platform serial device.");
        goto out;
    }

    printf(CPUSERVP"spawn_thread: Server thread binded well. at public EP %d\n",
           get_cpu_server()->server_ep_obj.cptr);
    return 0;

out:
    printf("spawn_thread: Server ran into an error.\n");
    if (get_cpu_server()->_badged_server_ep_cspath.capPtr != 0) {
        vka_cspace_free_path(parent_vka, get_cpu_server()->_badged_server_ep_cspath);
    }

    vka_free_object(parent_vka, &get_cpu_server()->server_ep_obj);
    return error;
}