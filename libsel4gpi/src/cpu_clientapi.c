...
int cpu_client_start(cpu_client_context_t *conn, void* vaddr, size_t size, seL4_CPtr frame_cap)
{ 
    seL4_SetMR(CPUMSGREG_FUNC, FUNC_START_REQ);
    seL4_SetMR(CPUMSGREG_START_REQ_VA, (seL4_Word) vaddr);
    seL4_SetMR(CPUMSGREG_START_REQ_SZ, (seL4_Word) size);
...
int cpu_client_config(cpu_client_context_t *conn, vka_t* vka, void* omit_vaddr, cpu_client_context_t *ret_conn)
{ 
    // Alloc a slot for the incoming cap.
    seL4_CPtr dest_cptr;
    ...
    seL4_SetMR(CPUMSGREG_CONFIG_REQ_OMIT_VA, (uintptr_t)omit_vaddr);
    seL4_MessageInfo_t tag = seL4_MessageInfo_new(0, 0, 0,
...
int cpu_client_rm(cpu_client_context_t *conn, void* vaddr, size_t size){
    return 0;
}

int cpu_client_bind_cpu(cpu_client_context_t *conn, seL4_CPtr cpu_cap) {
    return 0;
}