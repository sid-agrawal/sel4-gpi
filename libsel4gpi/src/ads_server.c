...
void ads_server_main()
{
    ...
    seL4_Error error = 0;
    ads_server_registry_entry_t *client_data = NULL;
    ...
    while (1) {
        ...
        case FUNC_CONNECT_REQ:
            printf(ADSSERVS"main: Got connect request from client badge %x.\n",
                    sender_badge);
            ...
            tag = seL4_MessageInfo_new(error, 0, 1, ADSMSGREG_CONNECT_ACK_END);
            reply(tag);
            break;
        ...
        case FUNC_ATTACH_REQ:
            ...
            /* Find the client */
            client_data = ads_server_registry_get_entry_by_badge(sender_badge);
            if (client_data == NULL) {
            ...
            tag = seL4_MessageInfo_new(error, 0, 0, ADSMSGREG_ATTACH_ACK_END);
            reply(tag);
            break;
