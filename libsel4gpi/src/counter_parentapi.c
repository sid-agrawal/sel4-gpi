                                  ...
                                  seL4_CPtr *server_ep_cap)
{
    ...
    memset(get_counter_server(), 0, sizeof(counter_server_context_t));

    /* Get a CPtr to the parent's root cnode. */
    vka_cspace_make_path(parent_vka, 0, &parent_cspace_cspath);

    get_counter_server()->server_vka = parent_vka;
    get_counter_server()->server_vspace = parent_vspace;
    get_counter_server()->server_cspace = parent_cspace_cspath.root;
    get_counter_server()->server_simple = parent_simple;

    /* Allocate the Endpoint that the server will be listening on. */
    error = vka_alloc_endpoint(parent_vka, &get_counter_server()->server_ep_obj);
    if (error != 0) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: failed to alloc endpoint, err=%d.",
                error);
        return error;
    }
    ...
    *server_ep_cap = get_counter_server()->server_ep_obj.cptr; 
    ...
    error = vka_mint_object(parent_vka, &get_counter_server()->server_ep_obj,
                            ...
                            seL4_AllRights,
                            get_counter_server()->parent_badge_value);
    if (error != 0) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: Failed to mint badged Endpoint cap to "
                "server.\n"
    ...
    error = sel4utils_configure_thread_config(parent_vka,
                                              ...
                                              parent_vspace,
                                              config,
                                              &get_counter_server()->server_thread);
    if (error != 0) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: sel4utils_configure_thread failed "
                "with %d.", error);
        goto out;
    }

    NAME_THREAD(get_counter_server()->server_thread.tcb.cptr, "counter server");
    error = sel4utils_start_thread(&get_counter_server()->server_thread,
                                   (sel4utils_thread_entry_fn)&counter_server_main,
                                   NULL, NULL, 1);
    if (error != 0) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: sel4utils_start_thread failed with "
                "%d.", error);
        goto out;
    }
     ...
     */
    seL4_SetMR(CSMSGREG_FUNC, FUNC_SERVER_SPAWN_SYNC_REQ);
    tag = seL4_MessageInfo_new(0, 0, 0, CSMSGREG_SPAWN_SYNC_REQ_END);
    tag = seL4_Call(get_counter_server()->_badged_server_ep_cspath.capPtr, tag);

    /* Did all go well with the server? */
    if (seL4_GetMR(CSMSGREG_FUNC) != FUNC_SERVER_SPAWN_SYNC_ACK) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: Server thread sync message after spawn "
    ...
    if (error != 0) {
        ZF_LOGE(COUNTERSERVP"spawn_thread: Server thread failed to bind to the "
...
out:
    ...
    vka_free_object(parent_vka, &get_counter_server()->server_ep_obj);
    return error;
}